...
class MLP_Model(nn.Module):
    def __init__(self, means=None, stds=None):
        ...
        means = torch.zeros((1, 330)) if means is None else torch.tensor(means, dtype=torch.float32).unsqueeze(0)
        stds = torch.ones((1, 330)) if stds is None else torch.tensor(stds, dtype=torch.float32).unsqueeze(0)
    ...
    def forward(self, x):
        ...
        if (torch.equal(self.means, torch.zeros_like(self.means))):
            warnings.warn("Value of 'means' is still default value. Double check if you actually set means")
        if (torch.equal(self.stds, torch.ones_like(self.stds))):
            warnings.warn("Value of 'stds' is still default value. Double check if you actually set stds")
