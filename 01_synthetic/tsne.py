...
def tsne(samples):
    ...
    for lattice_type in samples:
        particles = np.array([lattice.particles["Position"][:sample_len].flatten() for lattice in samples[lattice_type]]) # TODO determine why different samples have different number of atoms
